...
| TC-DN-07 | Downgrade uses the update endpoint | As a Pro user, schedule a downgrade while inspecting the browser Network panel and server logs | The UI calls `/api/subscriptions/update` with `items:[{product_id:...}]` and `update_behavior:"proration-none"`; the server action is `update` and Creem receives `POST /v1/subscriptions/{id}`, not the upgrade endpoint |
