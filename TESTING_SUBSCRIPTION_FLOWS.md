...
| ID | 场景 | 预期结果 |
| --- | --- | --- |
| TC-WH-03 | subscription.paid transaction dedup by transaction ID | Deliver paid events for different billing periods, then redeliver the same payment event with the exact same transaction ID (`last_transaction_id`/`transaction.id`/`payment_id`) | Redelivery with the same transaction ID creates no additional transaction row and adds no Credits. Different transaction IDs are treated as independent transactions even if they refer to the same subscription or billing cycle; such data must not be used for duplicate-payment regression testing |
...
| TC-WH-10 | `refund.created` official refund regression | Deliver an official-shape refund event with `object.refund.transaction_id` referencing an existing paid transaction and `object.refund.refund_amount` in positive cents, then redeliver the exact same event | One negative transaction is written with `amount=-refund_amount/100` (for example `2990` stores `-29.90`), inverse Credits, and `status=refunded`; user Credits are deducted once and clamped at zero. Redelivery creates no additional transaction and deducts no extra Credits |
...
| TC-WH-14 | Terminal event self-heals a deleted local subscription row | Delete only the local subscriptions row, then deliver subscription.canceled/expired/unpaid for that same real Creem subscription. Include metadata.user_id or customer.email when available; also repeat once with neither field to exercise the Creem subscription-ID lookup fallback | The webhook resolves the user from event identity, or by querying GET /v1/subscriptions?subscription_id=..., sees no remaining active subscription, resets users.plan=free and users.credits=0, and syncs customer metadata to free/0 with a fresh synced_at |
